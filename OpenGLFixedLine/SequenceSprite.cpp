...
void SequenceSprite::Init(const char*imageDir, const char *imageName, int startFrame, int EndFrame, int framePerSecond)
{
	...
	for (int i = startFrame;i <= EndFrame;++i)
	{
		...
		strcpy_s(szBuffer, imageDir);
		strcat(szBuffer, imageName);
		if (i < 10)
		{
			char temp[32] = { 0 };
			sprintf_s(temp, "000%d", i);
			strcat(szBuffer, temp);
		}
		else if (i >= 10 && i < 100)
		{
			char temp[32] = { 0 };
			sprintf_s(temp, "00%d", i);
			strcat(szBuffer, temp);
		}
		else if (i >= 100 && i < 1000)
		{
			char temp[32] = { 0 };
			sprintf_s(temp, "0%d", i);
			strcat(szBuffer, temp);
		}
		else if (i >= 1000)
		{
...
void SequenceSprite::SetRect(float x, float y, float width, float height)
{
...
void SequenceSprite::Update(float deltaTime)
{
	m_Time += deltaTime;
